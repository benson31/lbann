...
template <typename T>
static void dump_activations(data_type_layer<T> const& dtl,
                             std::string const& root_file_path,
                             std::string const& m_file_format,
                             std::string const& state_string)
{
  for (int i = 0; i < dtl.get_num_children(); ++i) {
    CircMatDT<T, El::Device::CPU> const circ_data(dtl.get_activations(i));
    if (circ_data.CrossRank() == circ_data.Root()) {
      auto const& data = circ_data.LockedMatrix();
      std::string const file_name =
        (root_file_path + state_string + "_" + dtl.get_name() + "_output" +
         std::to_string(i) + "." + m_file_format);
      if (m_file_format == "csv") {
        ...
        save_npz(file_name,
                 dtl.get_name() + "_output" + std::to_string(i),
...
void dump_outputs::do_dump_outputs(const model& m, const Layer& l)
{
  ...
  if (auto const* float_layer = dynamic_cast<const data_type_layer<float>*>(&l))
    dump_activations(*float_layer,
                     root_file_path,
                     m_file_format,
                     c.get_state_string());
  else if (auto const* dbl_layer =
             dynamic_cast<data_type_layer<double> const*>(&l))
    dump_activations(*dbl_layer,
                     root_file_path,
                     m_file_format,
                     c.get_state_string());
  else
    LBANN_ERROR("Unsupported data type.");
}
